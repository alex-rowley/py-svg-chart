...
def stylised_line_chart(x_shift=False, y_shift=False):
    ...
    line_chart = psc.LineChart(x_values=dates, y_values=[actual, expected], y_names=['Actual sales', 'Predicted sales'], x_max_ticks=30, x_label_format=x_labels, y_label_format=y_labels, width=1200, left_margin=100, right_margin=100, x_shift=x_shift, y_shift=y_shift)
...
def test_stylised_line_chart_without_shift():
    line_chart = stylised_line_chart(x_shift=False, y_shift=False)
    write_out(line_chart.render_with_all_styles(), name="detailed.svg")


def test_stylised_line_chart_with_x_shift():
    line_chart = stylised_line_chart(x_shift=True, y_shift=False)
    write_out(line_chart.render_with_all_styles(), name="detailed-x-shift.svg")


def test_stylised_line_chart_with_y_shift():
    line_chart = stylised_line_chart(x_shift=False, y_shift=True)
    write_out(line_chart.render_with_all_styles(), name="detailed-y-shift.svg")


def test_stylised_line_chart_with_xy_shift():
    line_chart = stylised_line_chart(x_shift=True, y_shift=True)
